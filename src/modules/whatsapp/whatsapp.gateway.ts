...
export class WhatsAppGateway implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit {
  ...
  emitExcelProgressToUser(userId: number, data: { excelId: number; progress: number; total: number; processed: number; status: string; filename?: string; message?: string; headers?: string[]; fromWhatsApp?: boolean }) {
    const userRoom = `user_${userId}`;
    this.server.to(userRoom).emit('excel-progress', data);
    this.logger.log(`📊 Progreso Excel emitido a usuario ${userId}: ${data.status} - ${data.progress?.toFixed(1) || 0}% ${data.message || ''}${data.fromWhatsApp ? ' (desde WhatsApp)' : ''}`);
  }
